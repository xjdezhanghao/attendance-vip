package com.attendance.project.system.user.service;

import com.attendance.common.constant.UserConstants;
import com.attendance.common.exception.ServiceException;
import com.attendance.common.utils.StringUtils;
import com.attendance.common.utils.bean.BeanValidators;
...
import com.attendance.framework.shiro.service.PasswordService;
...
import com.attendance.project.system.post.mapper.PostMapper;
import com.attendance.project.system.role.domain.Role;
import com.attendance.project.system.role.mapper.RoleMapper;
...
import com.attendance.project.system.user.mapper.UserPostMapper;
import com.attendance.project.system.user.mapper.UserRoleMapper;
...
public class UserServiceImpl implements IUserService
{
    ...
    @Override
    public String checkIdcardUnique(User user) {
        Long userId = StringUtils.isNull(user.getUserId()) ? -1L : user.getUserId();
        User info = userMapper.checkIdcardUnique(user.getIdcard());
        if (StringUtils.isNotNull(info) && info.getUserId().longValue() != userId.longValue())
        {
            return UserConstants.USER_IDCARD_NOT_UNIQUE;
        }
        return UserConstants.USER_IDCARD_UNIQUE;
    }
    ...
    @Override
    public List<User> selectUserByDeptRole(User user) {
        return userMapper.selectUserByDeptRole(user);
    }

    @Override
    public List<User> selectUserByDeptPost(User user) {
        return userMapper.selectUserByDeptPost(user);
    }
