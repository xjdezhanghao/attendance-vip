 ...
 */
@Service
public class PerfPostServiceImpl implements IPerfPostService
{
    @Autowired
    private PerfPostMapper postMapper;
    ...
    {
        List<PerfPost> userPosts = postMapper.selectPostsByUserId(userId);
        List<PerfPost> posts = postMapper.selectPostAll();
        for (PerfPost post : posts)
        {
            for (PerfPost userRole : userPosts)
            {
    ...
    public int deletePostByIds(String ids)
    {
        ...
        {
            PerfPost post = selectPostById(postId);
    ...
    public String checkPostNameUnique(PerfPost post)
    {
        ...
        PerfPost info = postMapper.checkPostNameUnique(post.getPostName());
    ...
    {
        Long postId = StringUtils.isNull(post.getPostId()) ? -1L : post.getPostId();
        PerfPost info = postMapper.checkPostCodeUnique(post.getPostCode());
        if (StringUtils.isNotNull(info) && info.getPostId().longValue() != postId.longValue())
        {
