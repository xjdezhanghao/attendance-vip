...
(function ($) {
    $.extend({
            ...
            responseHandler: function(res) {
                ...
                if (res.code == web_status.SUCCESS) {
                    if ($.common.isNotEmpty(table.options.sidePagination) && table.options.sidePagination == 'client') {
                        ...
                        var arr = []
                        if( table.options.modalName ==  "考勤记录" || table.options.modalName ==  "排班计划"){
                            //添加表头
                            for(var i = 0; i < res.rows.length; i++){

                                var arrtime = res.rows[i].split(',');
                                var obj = {};

                                for(var j = 0; j < table.options.columns.length; j++) {
                                    var key = table.options.columns[j].field;
                                    var a = '';
                                    var val='';

                                    var atdtime =arrtime[j].split(';');
                                    if (table.options.modalName ==  "考勤记录" ){
                                        if (atdtime[2]=="null"||(atdtime[2]==" |")){
                                            atdtime[2] = "";
                                        }
                                        if(atdtime[1]=="1"){//迟到
                                            val = atdtime[0]+' '+atdtime[2];
                                            a =  '<span style="color:#0000ff">'+val+'</span>'
                                            // a =  '<span style="color:#0000ff">'+atdtime[0]+' '+atdtime[2]+'</span>'
                                        }else if(atdtime[1]=="2"){//早退
                                            val = atdtime[0]+' '+atdtime[2];
                                            a =  '<span style="color: #ff0000;">'+val+'</span>'
                                        }else if(atdtime[1]=="0"){//科级修改
                                            /*val = atdtime[0]+' '+atdtime[2];*/
                                            val = atdtime[2];
                                            a =  '<span style="color：#00ff00 ;">'+val+'</span>'
                                            /* a =  '<span >'+val+'</span>'*/
                                        }else{
                                            val = atdtime[0].replace(" |","");
                                            a =  '<span >'+val+'</span>'
                                        }
                                    } else {
                                        val = atdtime[0];
            ...
            search: function(formId, tableId, pageNumber, pageSize) {
                ...
                if( table.options.modalName ==  "考勤记录" || table.options.modalName ==  "排班计划"){
                    table.options.columns =  [
                       ...
                       //添加表头
                        var stime0 = document.getElementById('startDate').value;
                        var etime0 = document.getElementById('endDate').value;
                    var stime;
                    var etime;
                    if(!!stime0){
                        stime = stime0;
                    ...
                    if (table.options.modalName ==  "考勤记录") {
                        ...
                        dateColumns.push("迟到");
                        dateColumns.push("早退");
                        dateColumns.push("核实出勤");
                        dateColumns.push("核实迟到");
                        dateColumns.push("核实早退");
                        ...
                        dateColumns.forEach(function (value, index) {
                            ...
                            /*  param.field = value;
                              param.title = value;*/
                            table.options.columns.push(param);
                        });
                    } else {
                        dateColumns.forEach(function (value, index) {
                            var param = {};
                    ...
                    btn1: function(index, layero){
                        ...
                        $.ajax({
                            ...
                            complete: function (result) {
                            	...
                            	if (table.options.modalName == "工资记录详情"){
                                    table.options.uploadComplete(result);
                                }
                            }
                        });
                    }
        ...
        common: {
            ...
            dateformatValid : function(text){
                var patten = new RegExp(/^\d{4}\-\d{2}\-\d{2}$/);
                return patten.test(text);
            },
            standardNumValid : function(text){
                var patten = new RegExp(/^$|^[\-\+]?\d+(\.\d+)?$/);
                return patten.test(text);
            },
            toStandardNum : function(text, bit){
                if (!bit) bit=2;
                var patten = new RegExp(/^$|^[\-\+]?\d+(\.\d+)?$/);
                var result = 0;
                if(patten.test(text) && text != ""){
                    result = parseFloat(parseFloat(text).toFixed(bit));
                }
                return result;
            },
        }
